...
gl::RenderPass::RenderPass() : handle(VK_NULL_HANDLE), depthAttachment{}, depthAttachmentRef{}
{
...
bool gl::RenderPass::Create(const gl::Device &device, const ShaderModules &modules)
{
    ...
    if (resolveAttachmentRefs.size() > 0) subpass.pResolveAttachments = resolveAttachmentRefs.data();

    if (HasDepthAttachment()) subpass.pDepthStencilAttachment = &depthAttachmentRef;
    ...
    if (HasDepthAttachment()) dependency.srcStageMask |= VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;

    dependency.srcAccessMask = 0;

    dependency.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    if (HasDepthAttachment()) dependency.dstStageMask |= VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;

    dependency.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
    if (HasDepthAttachment()) dependency.dstAccessMask |= VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;

    std::vector<VkAttachmentDescription> attachments;
    attachments.reserve(AttachmentsCount());
    for (size_t i = 0; i < colorAttachments.size(); i++) attachments.push_back(colorAttachments[i]);
    if (HasDepthAttachment()) attachments.push_back(depthAttachment);
    for (size_t i = 0; i < resolveAttachments.size(); i++) attachments.push_back(resolveAttachments[i]);
    ...
    if (!label.empty()) vk::SetObjectName(device.handle, (uint64_t)handle, VK_OBJECT_TYPE_RENDER_PASS, label);
...
{
    if (handle != VK_NULL_HANDLE) vkDestroyRenderPass(device.handle, handle, nullptr);
}
...
bool gl::RenderPass::HasDepthAttachment() const { return depthAttachment.format != VK_FORMAT_UNDEFINED; }
...
uint32_t gl::RenderPass::AttachmentsCount() const
{
    ...
    return static_cast<uint32_t>(colorAttachments.size() + dephtAttachment) +
           static_cast<uint32_t>(resolveAttachments.size());
}
