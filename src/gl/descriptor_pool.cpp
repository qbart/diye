...
void DescriptorSet::WriteUniformBuffer(uint32_t binding, const Buffer &buffer, VkDeviceSize offset, VkDeviceSize range)
{
    ...
    descriptorWrite.pBufferInfo     = &bufferInfos
                                       .back(); // pointers needs to updated later as the vector may cause reallocation
...
void DescriptorSet::WriteImage(uint32_t binding, const ImageView &imageView)
{
    ...
    descriptorWrite.pImageInfo      = &imageInfos
                                      .back(); // pointers needs to updated later as the vectore may cause reallocation
    descriptorWrite.pTexelBufferView = nullptr;
...
bool DescriptorPool::Create(const Device &device)
{
    ...
    if (!label.empty()) vk::SetObjectName(device.handle, (uint64_t)handle, VK_OBJECT_TYPE_DESCRIPTOR_POOL, label);
...
{
    if (handle != VK_NULL_HANDLE) vkDestroyDescriptorPool(device.handle, handle, nullptr);
}
...
void DescriptorPool::MaxSets(uint32_t count) { createInfo.maxSets = count; }
...
void DescriptorPool::UpdateDescriptorSet(const Device &device, uint32_t set)
{
    ...
    vkUpdateDescriptorSets(device.handle, static_cast<uint32_t>(ds.writes.size()), ds.writes.data(), 0, nullptr);
}
