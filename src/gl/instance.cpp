...
)
{
    ...
    {
        if (LogLevelType::Warning >= LogLevel) fmtx::Warn(pCallbackData->pMessage);
    }
    else if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT)
    {
        if (LogLevelType::Info >= LogLevel) fmtx::Info(pCallbackData->pMessage);
    }
    else if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT)
    {
        if (LogLevelType::Debug >= LogLevel) fmtx::Debug(pCallbackData->pMessage);
    }
...
{
    if (debugMessenger != VK_NULL_HANDLE) DestroyDebugUtilsMessengerEXT(handle, debugMessenger, nullptr);
