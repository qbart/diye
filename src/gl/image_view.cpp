...
bool ImageView::Create(const Device &device, const Image &image, VkFormat format)
{
    ...
    {
        if (!label.empty()) vk::SetObjectName(device.handle, (uint64_t)handle, VK_OBJECT_TYPE_IMAGE_VIEW, label);
...
void ImageView::AspectMask(VkImageAspectFlags flags) { createInfo.subresourceRange.aspectMask = flags; }

void ImageView::AspectMaskDepth() { AspectMask(VK_IMAGE_ASPECT_DEPTH_BIT); }

void ImageView::Destroy(const Device &device)
{
    if (handle != VK_NULL_HANDLE) vkDestroyImageView(device.handle, handle, nullptr);
}
} // namespace gl
