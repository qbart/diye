...
VkExtent2D SelectSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities, SDL_Window *window)
{
    if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max())
    {
        fmtx::Debug(
            fmt::format(
                "Using fixed Extent from surface capabilities {}x{}",
    ...
    else
    {
        ...
        fmtx::Debug(
            fmt::format(
                "Min image extent: {}x{}", capabilities.minImageExtent.width, capabilities.minImageExtent.height
            )
        );
        fmtx::Debug(
            fmt::format(
                "Max image extent: {}x{}", capabilities.maxImageExtent.width, capabilities.maxImageExtent.height
            )
        );
...
{
    if (handle != VK_NULL_HANDLE) vkDestroySwapchainKHR(device.handle, handle, nullptr);
}

VkResult
SwapChain::AcquireNextImage(const Device &device, uint32_t *imageIndex, VkSemaphore semaphore, VkFence fence) const
{
    return vkAcquireNextImageKHR(device.handle, handle, UINT64_MAX, semaphore, fence, imageIndex);
}

VkResult SwapChain::AcquireNextImageWithTimeout(
    const Device &device,
    uint32_t *imageIndex,
