...
public:
    ...
    void
    Circle(const Vec3 &center, const Vec3 &planeNormal = UP, float radius = 0.5f, const Vec3 &color = ORANGE) const;
    void Sphere(const Vec3 &p, float radius = 1.0f, const Vec3 &color = GREEN) const;
    void Plane(
        const Vec3 &origin,
        const Vec3 &normal      = UP,
        float size              = 1.0f,
        const Vec3 &color       = YELLOW,
        float normalSize        = 0,
        const Vec3 &normalColor = CYAN
    ) const;
    ...
    void Cylinder(
        const Vec3 &origin,
        const Vec3 &dir,
        const Vec3 &color = LIME,
        float radius      = 0.5f,
        float length      = 1.0f
    ) const;
    // void Text(const Camera &camera, const Vec3 &pos, const std::string &text, float scale = 1.0f, const Vec3 &color =
    // WHITE, const Vec2 &offset = Vec2(0, 0)) const;
