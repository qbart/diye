...
int Init()
{
    ...
    int res = SDL_Init(
        SDL_INIT_VIDEO | SDL_INIT_EVENTS | SDL_INIT_JOYSTICK | SDL_INIT_GAMECONTROLLER | SDL_INIT_HAPTIC
    );
    if (res != 0)
    {
        return res;
...
{
    if (wnd != nullptr) SDL_DestroyWindow(wnd);
}
...
void Window::PollEvents(UI *const ui)
{
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        if (ui != nullptr) ui->ProcessEvent(event);
    }
}
...
{
    if (KeyJustReleased(SDLK_ESCAPE)) Close();

    if (KeyDown(SDLK_w)) camera.MoveForward(5 * dt);

    if (KeyDown(SDLK_s)) camera.MoveBackward(5 * dt);

    if (KeyDown(SDLK_a)) camera.MoveLeft(5 * dt);

    if (KeyDown(SDLK_d)) camera.MoveRight(5 * dt);
