...
Camera::Camera() : zNear(0.1f), zFar(1000.0f), mode(Perspective), pixelsPerUnit(100), fov(60) {}

Camera::~Camera() {}

Vec3 Camera::ViewDir() const { return Mathf::Normalize(glm::conjugate(transform.rotation) * FORWARD); }
...
void Camera::UpdatePerspective(const Dimension &size) { SetPerspective(fov, size.w, size.h, zNear, zFar); }
...
void Camera::UpdateOrtho(const Dimension &size) { SetOrtho(size.w, size.h, pixelsPerUnit, zNear, zFar); }
...
Ray Camera::ScreenToRay(const Vec2 &screenPos, const Vec2 &screenSize) const
{
    Vec4 clipCoords = Vec4(
        (2.0f * screenPos.x) / screenSize.x - 1.0f, 1.0f - (2.0f * screenPos.y) / screenSize.y, -1.0f, 1.0f
    );
