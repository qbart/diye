...
Vec2 NormalizedVec2(float x, float y) { return glm::normalize(Vec2(x, y)); }

Vec3 NormalizedVec3(float x, float y, float z) { return glm::normalize(Vec3(x, y, z)); }

Vec2 Normalize(const Vec2 &v) { return glm::normalize(v); }

Vec3 Normalize(const Vec3 &v) { return glm::normalize(v); }

Vec3 Lerp(const Vec3 &a, const Vec3 &b, float t) { return glm::mix(a, b, t); }

float Lerp(float a, float b, float t) { return glm::mix(a, b, t); }

float Clamp(float val, float min, float max) { return glm::clamp(val, min, max); }

float Clamp01(float val) { return glm::clamp(val, 0.0f, 1.0f); }

float CloseTo(float a, float b, float epsilon) { return glm::epsilonEqual(a, b, epsilon); }

float Abs(float val) { return glm::abs(val); }

Quat QuatAngles(const Vec3 &angles)
{
    return glm::quat(glm::vec3(glm::radians(angles.x), glm::radians(angles.y), glm::radians(angles.z)));
}

Quat Rotate(const Quat &q, float deg, const Vec3 &axes) { return glm::rotate(q, glm::radians(deg), axes); }
