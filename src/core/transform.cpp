...
Transform::Transform() :
    localPosition(Vec3(0, 0, 0)), position(Vec3(0, 0, 0)), rotation(Quat(1, 0, 0, 0)), scale(Vec3(1, 1, 1))
{
    Update();
}
...
void Transform::Update() { matrix = ModelMatrix(Space::World); }
...
void Transform::Rotate(float angleDeg, const Vec3 &axis) { rotation = Mathf::Rotate(rotation, angleDeg, axis); }
