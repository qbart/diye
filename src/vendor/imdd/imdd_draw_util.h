...
#define IMDD_WIRE_CYLINDER_INDEX_COUNT (6 * IMDD_WIRE_CYLINDER_SEGMENT_COUNT)
    ...
    static void imdd_write_filled_box(void *vertex_base, uint32_t vertex_offset, uint16_t *indices)
    {
    ...
    static void imdd_write_filled_sphere(void *vertex_base, uint32_t vertex_offset, uint16_t *indices)
    {
        imdd_mesh_filled_vertex_t *vertices = (imdd_mesh_filled_vertex_t *)vertex_base + vertex_offset;
        for (uint32_t face = 0; face < 6; ++face)
        {
            ...
            for (uint32_t y = 0; y <= IMDD_FILLED_SPHERE_SUB; ++y)
                for (uint32_t x = 0; x <= IMDD_FILLED_SPHERE_SUB; ++x)
                {
                    ...
                    imdd_v4 const offset = imdd_v4_add(
                        imdd_v4_mul(tangent, imdd_v4_init_1f(tx)), imdd_v4_mul(bitangent, imdd_v4_init_1f(ty))
                    );
            ...
            // write triangles
            uint32_t const face_vertex_offset = vertex_offset +
                                                face * ((1 + IMDD_FILLED_SPHERE_SUB) * (1 + IMDD_FILLED_SPHERE_SUB));
            for (uint32_t y = 0; y < IMDD_FILLED_SPHERE_SUB; ++y)
                for (uint32_t x = 0; x < IMDD_FILLED_SPHERE_SUB; ++x)
                {
    ...
    static void imdd_write_filled_cone(void *vertex_base, uint32_t vertex_offset, uint16_t *indices)
    {
        imdd_mesh_filled_vertex_t *vertices = (imdd_mesh_filled_vertex_t *)vertex_base + vertex_offset;
    ...
    static void imdd_write_filled_cylinder(void *vertex_base, uint32_t vertex_offset, uint16_t *indices)
    {
        imdd_mesh_filled_vertex_t *vertices = (imdd_mesh_filled_vertex_t *)vertex_base + vertex_offset;
    ...
    static void imdd_write_wire_box(void *vertex_base, uint32_t vertex_offset, uint16_t *indices)
    {
        ...
        static uint8_t const idata[IMDD_WIRE_BOX_INDEX_COUNT] = {0, 1, 2, 3, 4, 5, 6, 7,
    ...
    static void imdd_write_wire_sphere(void *vertex_base, uint32_t vertex_offset, uint16_t *indices)
    {
        ...
        for (uint32_t face = 0; face < 6; ++face)
        {
            ...
            // write lines
            uint32_t const face_vertex_offset = vertex_offset +
                                                face * ((1 + IMDD_WIRE_SPHERE_SUB) * (1 + IMDD_WIRE_SPHERE_SUB));
            for (uint32_t y = 0; y <= IMDD_WIRE_SPHERE_SUB; ++y)
                for (uint32_t x = 0; x <= IMDD_WIRE_SPHERE_SUB; ++x)
                {
    ...
    static void imdd_write_wire_cone(void *vertex_base, uint32_t vertex_offset, uint16_t *indices)
    {
        imdd_mesh_wire_vertex_t *vertices = (imdd_mesh_wire_vertex_t *)vertex_base + vertex_offset;
    ...
    static void imdd_write_wire_cylinder(void *vertex_base, uint32_t vertex_offset, uint16_t *indices)
    {
        imdd_mesh_wire_vertex_t *vertices = (imdd_mesh_wire_vertex_t *)vertex_base + vertex_offset;
    ...
    static void imdd_mesh_layout_init(imdd_mesh_layout_t *mesh_layout, imdd_style_enum_t style)
    {
    ...
    static void imdd_mesh_layout_write(imdd_mesh_layout_t const *mesh_layout, void *vertices, uint16_t *indices)
    {
        for (uint32_t i = 0; i < IMDD_MESH_COUNT; ++i)
        {
            ...
            {
                mesh_desc->write_mesh_func(vertices, mesh_offsets->vertex_offset, indices + mesh_offsets->index_offset);
            }
        }
    }
...
#define IMDD_SHAPE_BUCKET_COUNT (IMDD_SHAPE_COUNT << 3)
    ...
    static inline uint32_t imdd_bucket_index_from_shape_header(imdd_shape_header_t header)
    {
        union
        {
    ...
    static inline imdd_shape_header_t imdd_shape_header_from_bucket_index(uint32_t index)
    {
        union
        {
            imdd_shape_header_t header;
    ...
    static void imdd_emit_line(imdd_wire_vertex_stream_t *stream, uint32_t col, imdd_v4 const *data)
    {
    ...
    static void imdd_emit_wire_triangle(imdd_wire_vertex_stream_t *stream, uint32_t col, imdd_v4 const *data)
    {
    ...
    static void imdd_emit_filled_triangle(imdd_filled_vertex_stream_t *stream, uint32_t col, imdd_v4 const *data)
    {
        ...
        imdd_v4 const normal = imdd_v4_normalize3(imdd_v4_cross(imdd_v4_sub(pos_c, pos_a), imdd_v4_sub(pos_a, pos_b)));
    ...
    static void imdd_emit_aabb(imdd_instance_stream_t *stream, uint32_t col, imdd_v4 const *data)
    {
    ...
    static void imdd_emit_transform(imdd_instance_stream_t *stream, uint32_t col, imdd_v4 const *data)
    {
        if (stream->current == stream->end)
        {
            return;
    ...
    static void imdd_emit_sphere(imdd_instance_stream_t *stream, uint32_t col, imdd_v4 const *data)
    {
        if (stream->current == stream->end)
        {
            return;
    ...
    static inline uint32_t imdd_instance_batch_index(
        imdd_mesh_enum_t mesh,
    ...
    static inline uint32_t imdd_array_batch_index(imdd_blend_enum_t blend, imdd_zmode_enum_t zmode)
    {
